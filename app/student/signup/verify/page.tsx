...
export default function StudentVerifyPage() {
  ...
  const verify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const created = await createProfile({ phone, name, role: "student", stage, grade });
      if (!created) {
        setMsg("فشل حفظ البروفايل");
        return;
      }
      // mark dev session and redirect to student profile
  ...
  return (
    <div className="mx-auto max-w-md p-6">
      <h1 className="text-2xl font-extrabold">تحقق تسجيل الطالب</h1>
      <form className="mt-4 space-y-3" onSubmit={verify}>
        ...
        <div>
          <label className="text-sm font-bold">اسم الطالب</label>
          <input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-md border px-3 py-2" />
        </div>
        <div>
          <label className="text-sm font-bold">المرحلة</label>
          <input value={stage} onChange={(e) => setStage(e.target.value)} className="w-full rounded-md border px-3 py-2" />
        </div>
        <div>
          <label className="text-sm font-bold">الصف</label>
          <input value={grade} onChange={(e) => setGrade(e.target.value)} className="w-full rounded-md border px-3 py-2" />
        </div>
        <div>
