...
export default function StudentSignupPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setMsg(null);
    try {
      // Dev flow: generate OTP, store in localStorage and print to console
      const code = Math.floor(100000 + Math.random() * 900000).toString();
      localStorage.setItem(`dev_otp_${phone}`, code);
      console.log("DEV OTP for", phone, "=", code);
      // store temporary name as well so verify page can prefill
      localStorage.setItem(`dev_name_${phone}`, name);
      setMsg("رمز التحقق تم إنشاؤه (اطلع على Console للرمز في بيئة التطوير).");
      // navigate to verify page
      const router = useRouter();
      router.push(`/student/signup/verify?phone=${encodeURIComponent(phone)}`);
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-md p-6">
      <h1 className="text-2xl font-extrabold">تسجيل طالب جديد</h1>
