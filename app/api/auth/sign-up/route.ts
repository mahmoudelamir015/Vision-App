...
export async function POST(request: Request) {
  ...
  if (settings && !settings.registration_open) return NextResponse.json({ error: "التسجيل مغلق حالياً" }, { status: 403 });
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    options: {
      data: {
        ...
        parent_phone: normalizeEgyptianPhone(typeof body.parent_phone === "string" ? body.parent_phone : ""),
        subjects: Array.isArray(body.subjects) ? body.subjects.filter((item): item is string => typeof item === "string") : [],
        student_code: typeof body.student_code === "string" ? body.student_code : null,
      },
    },
  });
  if (error || !data.user) return NextResponse.json({ error: "تعذر إنشاء الحساب بهذا الرقم" }, { status: 400 });
