...
export function QuestionBankModal({
  ...
  return (
    <div className="pointer-events-auto fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
      <div className="relative z-[61] flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-6 py-5 dark:border-slate-800 dark:bg-slate-800/30">
          <h2 className="flex items-center gap-2 text-xl font-bold text-slate-800 dark:text-white">
            <BookOpen className="h-6 w-6 text-vision-navy dark:text-vision-gold" />
            ...
            questions.map((question) => (
              <label
                ...
                className={`flex cursor-pointer items-start gap-4 rounded-2xl border-2 p-5 transition-all ${
                  selectedIds.includes(question.id)
                    ? 'border-vision-gold bg-vision-gold/5 shadow-sm dark:bg-vision-gold/10'
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="mb-4 text-lg font-bold text-slate-800 dark:text-slate-200">
                    {question.text}
                  </p>
                      ...
                      <div
                        ...
                        className={`rounded-lg border px-3 py-2 ${
                          optionIndex === question.correctAnswer
                            ...
                            : 'border-slate-100 bg-slate-50 text-slate-600 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-400'
                        }`}
                      >
                        {option}
                        {optionIndex === question.correctAnswer ? (
        ...
        <div className="flex items-center justify-between border-t border-slate-100 bg-slate-50 px-6 py-5 dark:border-slate-800 dark:bg-slate-800/50">
          <span className="font-bold text-slate-600 dark:text-slate-300">
            تم تحديد <span className="text-vision-navy dark:text-vision-gold">{selectedCount}</span>{' '}
            سؤال
          </span>
          <div className="flex gap-3">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl px-6 py-2.5 font-bold text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-300 dark:hover:bg-slate-700"
            >
              إلغاء
            </button>
            <button
              ...
              className="rounded-xl bg-vision-navy px-6 py-2.5 font-bold text-white shadow-md transition-colors hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-vision-gold dark:text-vision-navy dark:hover:bg-vision-gold-hover"
            >
              إدراج الأسئلة
            </button>
          </div>
